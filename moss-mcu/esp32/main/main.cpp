...
static void add_capabilities(cJSON *data) {
    ...
    const char *names[] = {
        ...
        "safety.link_watchdog", "light.red_eye_pwm", "display.ssd1306",
        "sensor.ina219", "sensor.aht20", "ir.raw_learn", "ir.raw_replay"
    };
...
static void handle_command(cJSON *root) {
    ...
    // Any syntactically valid command from the RDK counts as a link heartbeat.
    touch_heartbeat();

    if (strcmp(action, "system.heartbeat") == 0) {
    ...
    if (strcmp(action, "system.estop_clear") == 0) {
        ...
        if (!g_state.watchdog_stopped) g_motion.resume();
        g_peripherals.set_eye(0.35f);
        set_status_led(false);
    ...
    if (strcmp(action, "head.center") == 0) {
        ...
        if (!g_motion.center(speed)) {
            send_error(id, "MOTION_QUEUE", "unable to queue center motion");
            return;
        }
        send_ok_with_state(id);
        return;
    ...
    if (strcmp(action, "head.move") == 0) {
        ...
        if (!g_motion.enqueue(yaw, pitch, speed)) {
            send_error(id, "MOTION_QUEUE", "unable to queue head motion");
            return;
        }
        cJSON *out = response_base(id, true);
        cJSON *data = cJSON_AddObjectToObject(out, "data");
    ...
    if (strcmp(action, "display.text") == 0) {
        ...
        if (!g_peripherals.status().oled) {
            send_error(id, "DEVICE_UNAVAILABLE", "SSD1306 OLED not detected on I2C bus");
            return;
        }
        if (!g_peripherals.display_text(text.substr(0, 128))) {
            send_error(id, "DISPLAY_ERROR", "failed to update OLED");
            return;
        }
        cJSON *out = response_base(id, true);
        cJSON *data = cJSON_AddObjectToObject(out, "data");
    ...
    if (strcmp(action, "ir.learn") == 0) {
        if (!g_ir.available()) {
            send_error(id, "DEVICE_UNAVAILABLE", "IR RMT service is unavailable");
            return;
        }
        ...
        size_t count = 0;
        if (!g_ir.learn(slot, timeout_ms, &count)) {
            send_error(id, "IR_LEARN_FAILED", "no valid IR frame captured or slot table full");
            return;
        }
        cJSON *out = response_base(id, true);
        cJSON *data = cJSON_AddObjectToObject(out, "data");
    ...
    if (strcmp(action, "ir.send") == 0) {
        ...
        if (!g_ir.send(slot, repeat)) {
            send_error(id, "IR_SEND_FAILED", "slot not learned or transmit failed");
            return;
        }
        cJSON *out = response_base(id, true);
        cJSON *data = cJSON_AddObjectToObject(out, "data");
...
static void physical_estop_task(void *) {
    ...
    while (true) {
        const bool pressed = gpio_get_level(static_cast<gpio_num_t>(CONFIG_MOSS_ESTOP_GPIO)) == 0;
        if (pressed) {
